...
export default class Ray {
   ...
   */
  static makeRay(x: number, y: number, camHeight: number, camWidth: number,
    ...
    const width = ((x / (camWidth - 1)) * 2 - 1) * camera.near * Math.tan(fov) * camera.aspect;
    const height = ((y / (camHeight - 1)) * 2 - 1) * camera.near * Math.tan(fov);
