...
export default class RayVisitor implements Visitor {
  ...
  render(
    ...
    for (let x = 0; x < width; x++) {
      for (let y = 0; y < height; y++) {
        ...
        for (let shape of this.objects) { //This.objects wird in der Visit Methode gefüllt (enhält sphere und aabox)
          const intersection = shape.intersect(ray);
          if (intersection && intersection.closerThan(minIntersection)) { // if intersection != null
  ...
  visitSphereNode(node: SphereNode) {
    ...
    let sphere = new Sphere(m.mul(node.center), node.radius * scale, node.color); 
  ...
  visitCameraNode(node: CameraNode) {
    ...
    this.camera = new CameraNode(node.eye, node.center, node.up, node.fovy, node.aspect,
      node.near, node.far);
