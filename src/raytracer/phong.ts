...
function diffuseLightning(lightPositions: Array<LightNode>, kD:number, n:Vector, lightColor:Vector, p:Vector, phongColor:Vector){
  let I_d;
...
function specularLightning(lightPositions: Array<LightNode>, kS:number,lightColor:Vector, phongColor:Vector, v:Vector, p:Vector, n:Vector,shininess:number){
  for(const lightPosition of lightPositions){
    const s = lightPosition.position.sub(p).normalize();
